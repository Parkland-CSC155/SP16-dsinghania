...
var rawFolder = path.join(_dir, "raw");
var processedFolder = path.join(_dir, "processed");

var files = fs.readdirSync(rawFolder);
...
files.forEach(function(file) {
    ...
    if(file.startsWith("2014")) {
        fs.renameSync(oldPath, path.join(processedFolder, "2014", file));
    }
     if(file.startsWith("2015")) {
        fs.renameSync(oldPath, path.join(processedFolder, "2015", file));
    }
     if(file.startsWith("2016")) {
        fs.renameSync(oldPath, path.join(processedFolder, "2016", file));
    }
});

//counts
var cnt2014 = fs.readdirSync(path.join(processedFolder, "2014")).length;
var cnt2015 = fs.readdirSync(path.join(processedFolder, "2015")).length;
var cnt2016 = fs.readdirSync(path.join(processedFolder, "2016")).length;
